...
def learn_panda_to_vector(df, scaler):
	...
	if date_index != -1:
		...
		trans_table_date, trans_table_date_rev, X = vectorize(X, date_index)
...
def data_to_vector(data, scaler, trans_table_date, trans_table_hours, trans_table_item):
	dtn = np.asarray([data])
	if date_index != -1:
		dtn, _, _ = apply_transform(dtn, trans_table_date, date_index)
...
def reverse_vector(vec, scaler, trans_table_date_rev, trans_table_hours_rev, trans_table_item_rev):
	if use_scaler:
		...
		unscaled_vec = vec
	if date_index != -1:
		unscaled_vec[date_index] = trans_table_date_rev[int(unscaled_vec[date_index])]
	if hours_index != -1:
...
def main():
	...
	trans_table_date, trans_table_date_rev, trans_table_hours, trans_table_hours_rev, trans_table_item, trans_table_item_rev, Xgt = learn_panda_to_vector(gtn, sscaler)
	...
	if date_index != -1:
		dtn_transformed, trans_table_date, trans_table_date_rev = apply_transform(dtn_transformed, trans_table_date, date_index)
...
#		print("(d = "+str(sim_score)+")", rev_sim_vec)
	...
	for i in range(0, 5):#dtn.shape[0]
		...
		input_data = dtn[i]
		input_vec = data_to_vector(input_data, sscaler, trans_table_date, trans_table_hours, trans_table_item)
	...
	for res in result:
		...
		for i in range(0, len(closest_vecs)):
			...
			closest_entry = reverse_vector(closest_vec, sscaler, trans_table_date_rev, trans_table_hours_rev, trans_table_item_rev)
			print("#"*50)
...
if __name__ == "__main__":
	print("################################################")
	print("## TODO : vecotrize date, meaning that '2011/12/01' => 2011, 12, 01, i thing it gonna increase accuracy since scalarized date have nothing in common")
	print("################################################")
	main()
